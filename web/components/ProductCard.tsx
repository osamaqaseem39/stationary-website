        ...
        >
          ...
          <div className="relative aspect-[3/4] bg-gray-50 overflow-hidden">
            {image ? (
              <>
                ...
                <button
                  ...
                  className={`absolute top-2 right-2 p-2 rounded-full transition-all duration-300 z-20 ${inWishlist
                      ? 'bg-red-50 text-red-500 hover:bg-red-100'
                      : 'bg-white/80 backdrop-blur-sm text-gray-500 hover:bg-white hover:text-red-500 hover:shadow-md'
                    } ${!inWishlist && !isHovered ? 'lg:opacity-0 lg:translate-x-2' : 'opacity-100 translate-x-0'}`}
                ...
                <div className={`absolute inset-0 flex items-center justify-center pointer-events-none transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
                  <div
                    ...
                    className="pointer-events-auto cursor-pointer bg-white text-gray-900 px-4 py-2 rounded-full font-semibold shadow-lg hover:bg-gray-900 hover:text-white transition-colors transform hover:scale-105 active:scale-95 text-xs tracking-wide uppercase"
                  >
                    Quick View
                  </div>
                </div>
                ...
                {labels.length > 0 && (
                  <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
                    {labels.map((label, index) => {
                      ...
                      else if (labelLower.includes('limited')) bgClass = 'bg-purple-600'

                      return (
                        <span key={index} className={`${bgClass} text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm uppercase tracking-wider`}>
                          {label}
                        </span>
                      )
                    })}
            ...
            {isOutOfStock && (
              <div className="absolute inset-0 bg-white/60 flex items-center justify-center z-10">
                <span className="bg-gray-900 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Out of Stock</span>
              </div>
            )}
          </div>
          ...
          <div className="p-3">
            <div className="flex justify-between items-start gap-2 mb-1">
              <h3 className="text-sm font-medium text-gray-900 leading-tight line-clamp-2 group-hover:text-primary transition-colors">
                {name}
              </h3>
            </div>
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <button
                ...
                className={`p-2 rounded-full transition-all duration-300 ${isOutOfStock
                    ...
                    : 'bg-gray-100 text-gray-900 hover:bg-primary hover:text-white hover:shadow-lg hover:-translate-y-0.5'
                  }`}
                title={buttonText}
              >
